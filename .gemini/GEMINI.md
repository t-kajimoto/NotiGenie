...
- **コミットメッセージの禁則文字:** `# NotiGenieプロジェクト Gemini行動指針
...
記号は、シェルのコマンド置換を引き起こし、`run_shell_command`ツールがセキュリティエラーで失敗する原因となります。そのため、**コミットメッセージ内では`# NotiGenieプロジェクト Gemini行動指針
...
記号を絶対に使用しないこと。** ツールを呼び出す前に、生成したメッセージに`# NotiGenieプロジェクト Gemini行動指針
