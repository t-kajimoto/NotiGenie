...
class GeminiAdapter(ILanguageModel):
    ...
    async def perform_research(self, user_utterance: str, current_date: str, history: List[Dict[str, Any]] = None) -> str:
        ...
        system_instruction = f"""ユーザーの要求（タスク登録、買い物、献立作成など）をより便利で詳細なものにするために、必要な情報をGoogle検索で積極的に調査してください。
本日付: {current_date}

調査・補完すべき情報の例:
- 映画/イベント: 上映劇場、開催場所、具体的な日時、チケット予約開始日、公式サイトのURL
- 買い物/商品: 発売日、想定価格、店舗在庫状況、評価の高いモデルの型番
- 場所/レストラン: 住所、営業時間、定休日、電話番号、最寄り駅、予約サイト
- その他: ユーザーが語った背景（理由や目的）を達成するために役立つあらゆる事実情報

ユーザーから明示的に「調べて」と言われなくても、Notionの「メモ」を充実させるために有益だと判断した場合は、積極的に調査を実行してください。
調査結果を日本語で、箇条書きなどを用いて分かりやすく要約して回答してください。
どうしても調査の余地がない場合のみ「調査不要」と回答してください。"""
